...
class GameSettings extends Phaser.Scene {
  ...
  create() {
    ...
    musicRectangle = this.add.rectangle(230, 150, 275, 52, 0x6666ff).setInteractive({ cursor: 'pointer' })
      .on('pointerup', () => {
        globalState.music = !globalState.music;
      });
    this.add.bitmapText(230, 150, 'press-start-2p', 'GameMusic', 28).setOrigin(0.5);

    effectsRectangle = this.add.rectangle(230, 230, 225, 52, 0x6666ff).setInteractive({ cursor: 'pointer' })
      .on('pointerup', () => {
        globalState.effects = !globalState.effects;
      });
    this.add.bitmapText(230, 230, 'press-start-2p', 'GameSFX', 28).setOrigin(0.5);
